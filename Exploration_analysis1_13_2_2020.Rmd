...
regen_by_plotallspecies <- regen_by_plotallspecies %>% 
  mutate(tph = count * 1000)
View(regen_by_plotallspecies)
```
...
Now i will find regen per gap all species 
```{r regen br gap all species}
regen_by_gapallspecies <- regen_by_plotallspecies %>% 
     group_by(gap, spp) %>%
    summarise(tphg = sum(tph), mean_density = mean(tph), sem_density = sd(tph)/sqrt(n()))
View(regen_by_gapallspecies)
```
...
DAsummarySplot <- regen_by_plot2S %>%
  filter(regn_type =="N") %>% 
  select(gap, plot, spp, damage) %>%
  group_by(gap,plot, spp, damage) %>% 
  distinct() %>%
  filter(damage!="NA") %>% # this part was to remove all plots with NA values*** 
  summarise(count=n()) %>%
  mutate(Count_total = sum(count)) %>%
...
Damage_final <- DAsummarySplot %>% left_join(DAsummaryNplots)
view(Damage_final)
```
...
```{r}
Demdata_vrs_regenS <- Demdata %>% left_join(regen_data1N) %>% left_join(regen_by_plot1) %>% left_join(HsummaryS) %>%  
  filter(gap_type =="S")
view(Demdata_vrs_regenS)

Demdata_vrs_regenN <- Demdata %>% left_join(regen_data1N) %>% left_join(regen_by_plot1) %>% left_join(HsummaryN) %>% 
  filter(gap_type =="N")
...
ggplot(Aspect_vrs_varS, aes(aspect_dem11T, tph))+
  geom_point()+
  theme_classic()+
...
ggplot(Aspect_vrs_varS, aes(aspect_dem21T, tph))+
  geom_point()+
  theme_classic()+
  labs(title="Salvage", x="Aspect", y ="Total trees per hectare")
...
ggplot(Aspect_vrs_varS, aes(aspect_dem51T, tph))+
  geom_point()+
  theme_classic()+
  labs(title="Salvage", x="Aspect", y ="Total trees per hectare")
...
ggplot(Aspect_vrs_varS, aes(aspect_dem101T, tph))+
  geom_point()+
  theme_classic()+
  labs(title="Salvage", x="Aspect", y ="Total trees per hectare")
...
ggplot(Aspect_vrs_varN, aes(aspect_dem11T, tph))+
  geom_point()+
  theme_classic()+
...
ggplot(Aspect_vrs_varN, aes(aspect_dem21T, tph))+
  geom_point()+
  theme_classic()+
  labs(title="Natural", x="Aspect", y ="Total trees per hectare")
...
ggplot(Aspect_vrs_varN, aes(aspect_dem51T, tph))+
  geom_point()+
  theme_classic()+
  labs(title="Natural", x="Aspect", y ="Total trees per hectare")
...
ggplot(Aspect_vrs_varN, aes(aspect_dem101T, tph))+
  geom_point()+
  theme_classic()+
  labs(title="Natural", x="Aspect", y ="Total trees per hectare")
...
```{r}
AlldataS <- regen_by_plot1 left_join(regen_data1N) %>% left_join(HsummaryS) %>% left_join(GsummaryS) %>%  left_join(DsummaryS) %>% left_join(WsummaryS) 
```

Now we find other plots 
```{r}
notfd <- regen_data %>% 
  filter(spp !="Fd") %>% 
   mutate_at(.vars = vars( height, growth, diameter, width, damage, 
  ...
  mutate(count = 0)%>%
   distinct() %>% 
  select(gap, plot, gap_type, count)
view(notfd)

Now lets find the different species in all the plots 
```{r}
notfd_or_0 <- bind_rows(zero_counts1, notfd) %>%
  distinct()
view(notfd_or_0)
