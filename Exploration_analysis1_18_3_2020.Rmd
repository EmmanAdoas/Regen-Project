...
Growth summary 
```{r Summary of growth var}
Gr_summary_T <- HsummaryT %>% left_join(GsummaryT) %>% left_join(DsummaryT) %>% left_join(WsummaryT)
view(Gr_summary_T)
```

Writing csv for growth variables
```{r}
NMDS_gr_Fd <- Gr_summary_T %>% left_join(BAF_final) %>% left_join(Hemi_variables) %>% left_join(Underground_final)%>%  left_join(Neardist_final) %>%  left_join(Demdata_final) %>% left_join(gapsize)
view(NMDS_gr_Fd)


write.csv(NMDS_gr_Fd, "NMDS_gr_Fd.csv")
```
