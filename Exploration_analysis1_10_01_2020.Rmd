...
Receiving codes for the box plots from Mimi Baby
```{r}
Change X variables to factor, while y variable is numeric()
Variablename<- as.factor(Variablename)
boxplot(Yvariable~ Xvariable, main="The title of plot", xlab="name of X axis", ylab = "name of y axis", las=1)
` Barlettes test to check for equal or unequal variance
bartlett.test(Yvariable ~ Xvariable)
`t.test for equal variance
t.test(yvariable ~ xvariable, var.equal = TRUE)
`t.test for unequal variance
t.test(yvariable ~ xvariable, var.equal= FALSE)
