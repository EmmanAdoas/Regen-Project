...
select(gap, plot, gap_type, vrp_baf3, spp, n, baf)
view(BAF_final)
```
...
morrtSnewG <- mortSnew %>% 
group_by(gap, plot, spp, new_status) %>%
summarise(count= n()) %>% 
  filter(new_status=="0") %>% 
  mutate(tph = count * 1000)
View(morrtSnewG)
...
VsummarySplots <- regen_by_plot2S %>%
  filter(regn_type =="N") %>% 
  select(gap, plot, spp, vigour) %>%
  group_by(gap, plot, spp, vigour) %>% 
  summarise(count=n()) %>% 
  distinct() %>%
...
```{r}
NMDS_data <- regen_by_plotallspecies %>% left_join(BAF_final) %>% left_join(Hemi_variables) %>% left_join(Underground_final)%>%  left_join(Neardist_final) %>%  left_join(Demdata_final) %>% left_join(gapsize)
view(NMDS_data)
